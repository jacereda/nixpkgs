...
, jasper, gobject-introspection, doCheck ? false, makeWrapper, config }:
...
in stdenv.mkDerivation rec {
  ...
  buildInputs = stdenv.lib.optional config.allowXorg [ libX11 ];
  ...
  mesonFlags = [
    "-Ddocs=true"
    "-Djasper=true"
    "-Dx11=${if config.allowXorg then "true" else "false"}"
